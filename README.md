...
状态:v1 冻结基线完整闭环(M1+M2+M3+M4),PR #3 于 2026-07-02 合并(合并提交 `94e679d`)。此后 `main` 又经 Run 2(PR #7)、Run 2.5(PR #10)、M5 #12B(PR #13,合并提交 `c943c014`)、M6B(PR #16,合并提交 `efcb9d31`)、M7A.1(PR #18,合并提交 `3b857c0`)和 M7A.2(PR #20,合并提交 `4a0027a6`)推进,先后加入敌人感知模型、战斗反馈、"清空才可下楼"、Build 多样性 loadout(非末层清空后三选一、resolved 数值)、Encounter Diversity(房间 role + Grunt/Runner/Brute 三个 stat-only 敌人变体,composition-only)、M7A.1 真实可读性 HUD,以及 M7A.2 truthful per-enemy readability(逐敌人 identity + live HP)接进 gameplay。再往后 PR #21 与 PR #22 继续推进,`main` 当前 HEAD 为 `ff32004`(2026-07-24 合并),落地 M8 引擎无关模块基座(仓库根的 `m8_*.hpp`/`m8_*.cpp` 与配套 CTest 门)和 WARDENFALL 产品化;RC1 候选的验证结果(16/16 Core CTest、Development 与 Shipping 全量重建、Windows cook/stage/package、staged CSV 逐字节比对、Shipping 取证字符串扫描无命中)与仍未勾选的 owner checkpoint 见 [PORTFOLIO.md](PORTFOLIO.md) 与 [Docs/Release/RC-CHECKLIST.md](Docs/Release/RC-CHECKLIST.md),对应 tag 为 `wardenfall-rc1`。
...
本节记录 **M7A.2 / `4a0027a6`**(PR #20,code verification point / 合并提交)时的游戏状态;`main` 此后已推进到 `ff32004`,当前产品表面以 [PORTFOLIO.md](PORTFOLIO.md) 与 [Docs/Release/RC-CHECKLIST.md](Docs/Release/RC-CHECKLIST.md) 为准。Run 2([PR #7](https://github.com/My-Denia/uegame/pull/7),`playability`)带来随机但可复现的首局种子、出货用的关卡内置 spawner、provisional 平衡;Run 2.5([PR #10](https://github.com/My-Denia/uegame/pull/10),`combat-feel`,提交 `b932e21` + `7efdff0`)加入感知模型、战斗反馈、清空才可下楼;M5 #12B([PR #13](https://github.com/My-Denia/uegame/pull/13),`feat/m5-loadout-ue-binding`,合并提交 `c943c014`)把引擎无关的 Build 多样性 loadout 接进 gameplay——非末层清空后弹出三选一词缀,选择即改变 resolved 伤害/攻速/血量,reward-pending 在楼梯门前拦截下楼直到选择;末层不弹、直接走通关(取证动词随之到 18 个,见下);M6(Encounter Diversity;核心 [PR #15](https://github.com/My-Denia/uegame/pull/15),UE binding [PR #16](https://github.com/My-Denia/uegame/pull/16),合并提交 `efcb9d31`)把引擎无关的 m6 核心(房间 role + 敌人 archetype,composition-only)接进 gameplay——每层房间在冻结布局上获得 Quiet/Standard/Skirmish/Stronghold 语义 role,敌人按 role 加权成为 Grunt/Runner/Brute 三 stat 变体(Grunt 与 Default 行逐字段 parity;视觉只缩放 BodyMesh,胶囊/ContactRange 不动;数量、位置、spawnPlanHash/enemyPlanHash 全部保持 M5 基线,新增 roomRoleHash/enemyTypeHash 两锚;取证动词随之到 20 个,见下);M7A.1([PR #18](https://github.com/My-Denia/uegame/pull/18),`feat/m7a1-readability-hud`,合并提交 `3b857c0`)加入 code-only 的真实(truthful)AHUD/UCanvas overlay,把既有 M5/M6 运行时状态在正常游玩中直接可读——玩家 HP/MaxHP、resolved 伤害、attack_ms 与每秒攻击次数、已选词缀、RewardPending 与 1/2/3 三选一 offer、floor/runSeed、最近房间 role、Grunt/Runner/Brute tally、enemyTypeHash;无 UMG、无新美术、无玩法变化(取证动词不变,仍为 20 个;详见下方 M7A.1 段)。引擎无关生成核心(dungeon.hpp / m2_adapter.hpp)自 v1 起逐字节冻结——seed-7 的布局哈希锚点(ts100 `planHash`、ts200 `planHash`、tile 无关 `enemyPlan`)不动;敌人数量与逐层缩放改由下方当前 CSV 驱动。
...
**M7A.1 Truthful HUD / 可读性 HUD。** code-only 的 AHUD/UCanvas 双面板 overlay(左:玩家/Build——HP/MaxHP、resolved 伤害、attack_ms 与每秒攻击、已选词缀、RewardPending 时的 1/2/3 offer;右:run/encounter——floor/runSeed、最近房间 role、Grunt/Runner/Brute tally、enemyTypeHash)。HUD 直接读取既有 runtime truth source(LoadoutComponent / HealthComponent / FloorManager / DungeonSpawner 的 encounter cache),与 `Dungeon.LoadoutStatus`、`Dungeon.RoomRoles`、`Dungeon.EnemyRoster` 三个取证动词同源同 accessor——不复制、不伪造、不重算;encounter snapshot 与 FloorManager live 状态不一致(stale)时,HUD 只显示一行 stale 提示,绝不把旧的 role/roster/hash 当作当前值显示。`ui.ShowReadout`(默认 1)与 `ui.ShowReadoutScale` 控制显示与缩放。M7 系列边界:M7A.1 是 player/build/room/encounter aggregate HUD;M7A.2 是 per-enemy identity + live HP;M7A.3 floating damage numbers indefinitely deferred;M7B tint/material polish 仍 deferred。

**M7A.2 Enemy Readability / 敌人可读性。** PR #20(code verification point / 合并提交 `4a0027a6`)继续使用既有 code-only `AHUD/UCanvas`,为 live enemy 投影 archetype nameplate、实时 HP 数字和 screen-space HP bar。Grunt / Runner / Brute 身份是保存在 actor 上的 numeric `TypeId`;名称由 `EncounterConfig::TypeName(TypeId)` 派生,不保存 authoritative string;未分配 archetype 的 static/fallback enemy 显示 `Enemy`,不冒充 Grunt。HP 每帧直接读取 `UHealthComponent`,没有第二份 HP cache。渲染依次做距离、camera-forward、screen bounds 和 `WorldStatic` 遮挡过滤;默认最多 8 个 labels、16 次 occlusion traces/frame,遮挡后只在该有界预算内递补。label anchor 来自 `GetActorBounds(false)`,因此跟随 Runner / Grunt / Brute 的实际体型。逐敌 readout code path 编译进入 Shipping,且不受 `!UE_BUILD_SHIPPING` gate 排除;这项代码/构建事实不表示 current-main Shipping enemy-label/HP visual 已完成。实现没有 UMG、WidgetComponent、world-material tint 或 floating damage numbers,也没有 gameplay、AI、stats、RNG、enemy count 或 position 改动。

**取证动词(本快照 20 个,当前 44 个)**。Run 2.5 新增第 16 个 `Dungeon.AggroStatus`;PR #12B(M5 loadout)新增第 17、18 个 `Dungeon.LoadoutStatus` 与 `Dungeon.ChooseLoadout`;M6B(encounter UE binding)新增第 19、20 个 `Dungeon.RoomRoles`(runSeed/floor/floorSeed/roomRoleHash/每房 role)与 `Dungeon.EnemyRoster`(archetype 计数、每房 roster、resolved stats、enemyTypeHash + 保持不变的 m2 锚)(全部 shipping-gated,在 `!UE_BUILD_SHIPPING` 门内)。M8/WARDENFALL 之后动词继续增加:当前 `main` 上的 [DungeonEvidence.cpp](uegame/Source/uegame/DungeonEvidence.cpp) 共注册 44 个互不相同的 `Dungeon.*` 动词(新增 `Dungeon.Finale*`、`Dungeon.Route*`、`Dungeon.Contract*`、`Dungeon.TogglePause`、`Dungeon.ActiveThreatIds` 等),整份文件仍包在同一个文件级 `!UE_BUILD_SHIPPING` 门内(`#if` 在第 52 行,`#endif` 在第 1974 行)。下方 §7 的动词表列的仍是 v1/M4 的 15 个集合(冻结历史)。

**CI(Run 3 引入;本快照 8 门,当前 16 门)**。引擎无关的 g++ Core CTest 确定性门定义在 [CMakeLists.txt](CMakeLists.txt),由 [.github/workflows/core-ctest.yml](.github/workflows/core-ctest.yml) 在每个 pull request 与推送到 main 时跑完整 ctest 套件——此前这些门只能本地手动跑(CI 于 Run 3 引入时为 4 门,M5/M6 各追加确定性+golden 两门)。本快照时为 8 门(M1–M4 legacy:`m1_validate`、`m2_world_validate`、`m3_enemy_determinism`、`m4_floor_determinism`;M5:`m5_loadout_determinism`、`m5_loadout_goldenvec`;M6:`m6_encounter_determinism`、`m6_encounter_goldenvec`);M8 再追加 8 门(`m8_build_synergy`、`m8_enemy_behavior`、`m8_objective_compass`、`m8_build_routes`、`m8_room_flow`、`m8_finale`、`m8_runtime_authority`、`m8_presentation`),当前 CMakeLists.txt 共 16 个 `add_test`。
