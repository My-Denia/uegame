...
> **Portfolio RC update (2026-07-23):** The current candidate is productized as **WARDENFALL**: a complete three-floor structure, Executioner/Tempo/Bulwark builds, risk/reward room contracts, distinct enemies, a Warden finale, title/pause/confirmation/result flows, procedural audio, and a Windows Shipping package. See [PORTFOLIO.md](PORTFOLIO.md) and [Docs/Release/RC-CHECKLIST.md](Docs/Release/RC-CHECKLIST.md) for current facts, verification, and honest remaining boundaries. The milestone narrative below is retained as historical evidence and no longer represents the latest product surface.

Status: the v1 frozen baseline is a complete loop (M1+M2+M3+M4); PR #3 merged on 2026-07-02 (merge commit `94e679d`). `main` has since advanced through Run 2 (PR #7), Run 2.5 (PR #10), M5 #12B (PR #13, merge commit `c943c014`), M6B (PR #16, merge commit `efcb9d31`), M7A.1 (PR #18, merge commit `3b857c0`), and M7A.2 (PR #20, merge commit `4a0027a6`), adding — in order — an enemy perception model, combat feedback, clear-to-descend stairs, the Build-diversity loadout (a 3-choose-1 reward on each non-final floor clear, resolved stats), Encounter Diversity (room roles + Grunt/Runner/Brute stat-only enemy variants, composition-only), the M7A.1 truthful readability HUD, and M7A.2 truthful per-enemy readability (identity + live HP) now wired into gameplay. It has since advanced further through PR #21 and PR #22 to HEAD `ff32004` (merged 2026-07-24), landing the M8 engine-agnostic module foundation (the root `m8_*.hpp`/`m8_*.cpp` files and their CTest gates) and the WARDENFALL productization; the RC1 candidate's verification results (16/16 Core CTests, full Development and Shipping rebuilds, Windows cook/stage/package, byte-for-byte staged-CSV parity, a clean Shipping forensic string scan) and the still-unchecked owner checkpoint live in [PORTFOLIO.md](PORTFOLIO.md) and [Docs/Release/RC-CHECKLIST.md](Docs/Release/RC-CHECKLIST.md); the corresponding tag is `wardenfall-rc1`.
...
**M7A.1 truthful readability HUD.** A code-only AHUD/UCanvas two-panel overlay (left: player/build — HP/MaxHP, resolved damage, attack_ms with attacks-per-second, chosen affixes, and the 1/2/3 reward offer while RewardPending; right: run/encounter — floor/runSeed, nearest-room role, the Grunt/Runner/Brute tally, enemyTypeHash). The HUD reads the existing runtime truth sources directly (LoadoutComponent / HealthComponent / FloorManager / the DungeonSpawner encounter cache) — the same state, through the same accessors, as the `Dungeon.LoadoutStatus`, `Dungeon.RoomRoles`, and `Dungeon.EnemyRoster` forensic verbs; nothing is copied, faked, or recomputed. When the encounter snapshot is stale against the live FloorManager run/floor, the HUD shows a single stale marker and never presents the old role/roster/hash as current. `ui.ShowReadout` (default 1) and `ui.ShowReadoutScale` control visibility and scale. M7 boundaries: M7A.1 is the player/build/room/encounter aggregate HUD; M7A.2 is per-enemy identity + live HP; M7A.3 floating damage numbers are deferred indefinitely; M7B tint/material polish remains deferred.

**M7A.2 Enemy Readability.** PR #20 (code verification point / merge commit `4a0027a6`) continues to use the existing code-only `AHUD/UCanvas` and projects an archetype nameplate, live HP number, and screen-space HP bar over each live enemy. Grunt / Runner / Brute identity is the numeric `TypeId` stored on the actor; the name is derived through `EncounterConfig::TypeName(TypeId)`, never stored as an authoritative string; an unassigned static/fallback enemy displays `Enemy`, never a fabricated Grunt. HP is read directly from `UHealthComponent` every frame with no second HP cache. Rendering filters by distance, camera-forward, screen bounds, and `WorldStatic` occlusion; defaults cap work at 8 labels and 16 occlusion traces per frame, with refill only inside that bounded budget after an occluded candidate. The label anchor comes from `GetActorBounds(false)`, so it follows the actual Runner / Grunt / Brute body size. The per-enemy readout code path is compiled into Shipping and is not excluded by an `!UE_BUILD_SHIPPING` gate; this code/build statement does not claim that a current-main Shipping enemy-label/HP visual has been captured. The implementation adds no UMG, WidgetComponent, world-material tint, or floating damage numbers, and changes no gameplay, AI, stats, RNG, enemy count, or position.

**Forensic verbs (20 at this snapshot, 44 today).** Run 2.5 added the 16th, `Dungeon.AggroStatus`; PR #12B (M5 loadout) added the 17th and 18th, `Dungeon.LoadoutStatus` and `Dungeon.ChooseLoadout`; M6B (encounter UE binding) added the 19th and 20th, `Dungeon.RoomRoles` (runSeed/floor/floorSeed/roomRoleHash/per-room role) and `Dungeon.EnemyRoster` (archetype counts, per-room roster, resolved stats, enemyTypeHash + the unchanged m2 anchors) — all shipping-gated, inside `!UE_BUILD_SHIPPING`. Verbs kept growing through M8/WARDENFALL: on current `main`, [DungeonEvidence.cpp](uegame/Source/uegame/DungeonEvidence.cpp) registers 44 distinct `Dungeon.*` verbs (adding `Dungeon.Finale*`, `Dungeon.Route*`, `Dungeon.Contract*`, `Dungeon.TogglePause`, `Dungeon.ActiveThreatIds` and others), and the whole file still sits inside one file-wide `!UE_BUILD_SHIPPING` guard (`#if` at line 52, `#endif` at line 1974). The verb table in §7 below still lists the v1/M4 set of 15 (frozen history).

**CI (introduced in Run 3; 8 gates at this snapshot, 16 today).** The engine-agnostic g++ Core CTest determinism gates are defined in [CMakeLists.txt](CMakeLists.txt) and run as the full ctest suite on every pull request and push to main via [.github/workflows/core-ctest.yml](.github/workflows/core-ctest.yml) — previously these gates were only runnable locally (CI started with 4 gates in Run 3; M5/M6 each added a determinism+golden pair). At this snapshot there were 8 (M1–M4 legacy: `m1_validate`, `m2_world_validate`, `m3_enemy_determinism`, `m4_floor_determinism`; M5: `m5_loadout_determinism`, `m5_loadout_goldenvec`; M6: `m6_encounter_determinism`, `m6_encounter_goldenvec`); M8 added 8 more (`m8_build_synergy`, `m8_enemy_behavior`, `m8_objective_compass`, `m8_build_routes`, `m8_room_flow`, `m8_finale`, `m8_runtime_authority`, `m8_presentation`), for the 16 `add_test` gates in CMakeLists.txt today.
